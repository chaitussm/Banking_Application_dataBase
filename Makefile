...
.PHONY: db-init db-reset db-seed db-inspect db-verify db-unseed db-down db-rebuild db-report db-failure-report db-prepare-email db-publish-summary scripts-chmod

scripts-chmod:
	chmod +x scripts/init_db.sh scripts/verify_db.sh scripts/generate_db_report.sh scripts/generate_failure_report.sh scripts/generate_email_health_report.sh scripts/generate_email_failure_report.sh scripts/prepare_pipeline_email.sh scripts/publish_github_summary.sh
...
db-failure-report:
	$(MAKE) scripts-chmod
	bash scripts/generate_failure_report.sh "$(DB_FILE)"

db-prepare-email:
	$(MAKE) scripts-chmod
	bash scripts/prepare_pipeline_email.sh "$(DB_FILE)"
